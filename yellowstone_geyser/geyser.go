...
import (
	...
	"strconv"
	"sync"
	grpc_wee "transfer_monitor/grpc"
...
func (s *StreamClient) SubscribeTransaction(filterName string, txreq *yellowstone_geyser_pb.SubscribeRequestFilterTransactions) error {
	s.request.Transactions[filterName] = txreq
	return s.geyser.Send(s.request)
}

// SubscribeTransaction subscribes to transaction updates.
func (s *StreamClient) SubscribeMultiRequest(filterName string, txreq *yellowstone_geyser_pb.SubscribeRequestFilterTransactions, blockreq *yellowstone_geyser_pb.SubscribeRequestFilterBlocks) error {
	s.request.Transactions[filterName] = txreq
	s.request.Blocks[filterName] = blockreq
	return s.geyser.Send(s.request)
}
